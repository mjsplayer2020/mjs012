...
void MJSMjaiClient::Set_type_hora(MJSTkinfo *tk, MJSPlayer *ply, MJSGui *gui, int tmp_wk_num){
	...
	tk->kyoku[tk->kyoku_index].yk.yakucount = 0;

	// -----------------------------
	// 値定義
	// -----------------------------
	tk->kyoku[tk->kyoku_index].yk.yakucount     = 0;
	tk->kyoku[tk->kyoku_index].yk.yakuman_count = 0;
	tk->kyoku[tk->kyoku_index].yk.agari_ply_han = 0;
	tk->kyoku[tk->kyoku_index].yk.agari_ply_mangan_count = 0;
	tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai=0;

	// ----------------------------------------
	// 流局フラグ
	// ----------------------------------------
	tk->kyoku[tk->kyoku_index].yk.ryukyoku_flg = false;
	...
	for( int tmp_i = 0; tmp_i < PLAYER_MAX; tmp_i++ ) {
		...
		if (tmp_agari_ply_id == tk->ply_mjai_id[tmp_i]){
			tk->kyoku[tk->kyoku_index].yk.agari_ply_num = tmp_i;
		}

		// 振込プレーヤ番号の設定
		if (tmp_target_ply_id == tk->ply_mjai_id[tmp_i]){
			tk->kyoku[tk->kyoku_index].yk.furikomi_ply_num = tmp_i;
		}
	...
	if ( tmp_agari_ply_id == tmp_target_ply_id ){
		tk->stat = PLYTSUMOAGARI;
		tk->kyoku[tk->kyoku_index].yk.tsumo_agari_flg = true;
	}else{
		tk->stat = PLYACTRON;
		tk->kyoku[tk->kyoku_index].yk.tsumo_agari_flg = false;
	}
	...
	for(int tmp_i = 0; tmp_i < gui->wk_str_count-tmp_wk_num; tmp_i++){
		...
		if( yakus_read_mode == 0){
			...
			if(strcmp(gui->wk_str[tmp_wk_num+tmp_i], "pai") == 0 ){
				...
				// 和了牌登録
				tk->kyoku[tk->kyoku_index].yk.agari_hai = tmp_hai;
				tk->kyoku[tk->kyoku_index].yk.agari_aka = tmp_aka;
		...
		// -----------------------------
			...
			if(strcmp(gui->wk_str[tmp_wk_num+tmp_i], "yakus") == 0 ){
				...
				if ( tk->kyoku[tk->kyoku_index].agari_tehaicount < tk->ply_act_tehaicount[tk->kyoku[tk->kyoku_index].yk.agari_ply_num] ){
					// 牌番号取得
					tmp_hai = Get_hainum(gui->wk_str[tmp_wk_num+tmp_i]);
					// 赤牌確認
					if (tmp_hai > 100){
		...
		// -----------------------------
			...
			if(strcmp(gui->wk_str[tmp_wk_num+tmp_i], "fu") == 0 ){
				...
				// 符読み込み
				tk->kyoku[tk->kyoku_index].yk.scoreFu = atoi(gui->wk_str[tmp_wk_num+tmp_i+1]);
		...
		// -----------------------------
			...
			if(strcmp(gui->wk_str[tmp_wk_num+tmp_i], "fan") == 0 ){
				tk->kyoku[tk->kyoku_index].yk.agari_ply_han = atoi(gui->wk_str[tmp_wk_num+tmp_i+1]);
			}
			...
			if(strcmp(gui->wk_str[tmp_wk_num+tmp_i], "deltas") == 0 ){
				...
				for(int ply_i = 0; ply_i < PLAYER_MAX; ply_i++){
					tk->kyoku[tk->kyoku_index].yk.kyoku_score[(tmp_ply_id_0_seki + ply_i)%4] = atoi(gui->wk_str[tmp_wk_num+tmp_i+ply_i+1]);
				}
...
void MJSMjaiClient::Set_type_hora_yaku_check(MJSTkinfo *tk, MJSPlayer *ply, MJSGui *gui, int tmp_yaku_num){
	...
	if(strcmp(gui->wk_str[tmp_yaku_num], "reach") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[RIICH] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[RIICH] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[RIICH];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 02: ダブルリーチ
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "double_reach") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[WRIICH] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[WRIICH] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[WRIICH];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 03: 一発
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "ippatsu") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[IPPATSU] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[IPPATSU] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[IPPATSU];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 04: 清一色
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "chiniso") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[CHINISO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[CHINISO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[CHINISO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 05: 混一色
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "honiso") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[HONISO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[HONISO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[HONISO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 06: 純全帯
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "junchantaiyao") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[JUNCHAN] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[JUNCHAN] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[JUNCHAN];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 07: 二盃口
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "ryanpeko") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[RYANPEKO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[RYANPEKO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[RYANPEKO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 08: 混老頭
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "honroto") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[HONROTO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[HONROTO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[HONROTO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 09: 小三元
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "shosangen") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[SHOSANGEN] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[SHOSANGEN] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[SHOSANGEN];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 10: 一気通貫
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "ikkitsukan") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[ITSU] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[ITSU] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[ITSU];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 11: 三色同刻
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "sanshokudoko") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[SANDOKOU] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[SANDOKOU] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[SANDOKOU];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 12: 三色同順
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "sanshokudojun") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[SANDOJUN] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[SANDOJUN] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[SANDOJUN];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 13: 三槓子
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "sankantsu") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[SANKANTSU] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[SANKANTSU] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[SANKANTSU];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 14: 三暗刻
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "sananko") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[SANANKO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[SANANKO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[SANANKO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 15: 対々和
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "toitoiho") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[TOITOI] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[TOITOI] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[TOITOI];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 16: 混全帯
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "honchantaiyao") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[CHANTA] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[CHANTA] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[CHANTA];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 17: 七対子
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "chitoitsu") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[CHITOI] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[CHITOI] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[CHITOI];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 18: 平和
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "pinfu") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[PINFU] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[PINFU] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[PINFU];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 19: 断公九
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "tanyaochu") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[TANYAO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[TANYAO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[TANYAO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 20: 一盃口
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "ipeko") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[IPEKO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[IPEKO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[IPEKO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 21: 場風牌
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "bakaze") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[BAKAZE] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[BAKAZE] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[BAKAZE];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 22: 自風牌
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "jikaze") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[ZIKAZE] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[ZIKAZE] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[ZIKAZE];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 23: 三元牌
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "sangenpai") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[YAKUHAI] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[YAKUHAI] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[YAKUHAI];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 27: 海底摸月
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "haiteiraoyue") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[HAITEI] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[HAITEI] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[HAITEI];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 28: 河底撈魚
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "hoteiraoyui") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[HOUTEI] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[HOUTEI] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[HOUTEI];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 29: 嶺上開花
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "rinshankaiho") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[RINSHAN] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[RINSHAN] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[RINSHAN];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;
	...
	// 31: 門前清自摸和
		...
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[TSUMO] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[TSUMO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[TSUMO];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 32: ドラ(表ドラ)
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "dora") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[DORA] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[DORA] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han  = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[DORA];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 33: 裏ドラ
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "uradora") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[URADORA] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[URADORA] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han     = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[URADORA];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;

	// 34: 赤牌
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "akadora") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yaku_flg[AKAHAI] = true;
		tk->kyoku[tk->kyoku_index].yk.yaku_han[AKAHAI] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.agari_ply_han   = tk->kyoku[tk->kyoku_index].yk.agari_ply_han + tk->kyoku[tk->kyoku_index].yk.yaku_han[AKAHAI];
		tk->kyoku[tk->kyoku_index].yk.yakucount++;
	...
	// 役満01: 天和
		...
		tk->kyoku[tk->kyoku_index].yk.yakuman_flg[TENHO] = true;
		tk->kyoku[tk->kyoku_index].yk.yakuman_bai[TENHO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai = tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai + tk->kyoku[tk->kyoku_index].yk.yakuman_bai[TENHO];
		tk->kyoku[tk->kyoku_index].yk.yakuman_count++;

	// 役満02: 地和
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "chiho") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yakuman_flg[CHIHO] = true;
		tk->kyoku[tk->kyoku_index].yk.yakuman_bai[CHIHO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai = tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai + tk->kyoku[tk->kyoku_index].yk.yakuman_bai[CHIHO];
		tk->kyoku[tk->kyoku_index].yk.yakuman_count++;
	...
	// 役満07: 大三元
		...
		tk->kyoku[tk->kyoku_index].yk.yakuman_flg[DAISANGEN] = true;
		tk->kyoku[tk->kyoku_index].yk.yakuman_bai[DAISANGEN] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai = tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai + tk->kyoku[tk->kyoku_index].yk.yakuman_bai[DAISANGEN];
		tk->kyoku[tk->kyoku_index].yk.yakuman_count++;
	...
	// 役満15: 四暗刻
		...
		tk->kyoku[tk->kyoku_index].yk.yakuman_flg[SUANKO] = true;
		tk->kyoku[tk->kyoku_index].yk.yakuman_bai[SUANKO] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai = tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai + tk->kyoku[tk->kyoku_index].yk.yakuman_bai[SUANKO];
		tk->kyoku[tk->kyoku_index].yk.yakuman_count++;

	// 役満16: 人和(役満)
	}else if(strcmp(gui->wk_str[tmp_yaku_num], "renho") == 0 ){
		tk->kyoku[tk->kyoku_index].yk.yakuman_flg[RENHOYAKUMAN] = true;
		tk->kyoku[tk->kyoku_index].yk.yakuman_bai[RENHOYAKUMAN] = atoi(gui->wk_str[tmp_yaku_num+1]);
		tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai = tk->kyoku[tk->kyoku_index].yk.yakuman_agari_ply_bai + tk->kyoku[tk->kyoku_index].yk.yakuman_bai[RENHOYAKUMAN];
		tk->kyoku[tk->kyoku_index].yk.yakuman_count++;
	}
...
void MJSMjaiClient::Set_type_ryukyoku(MJSTkinfo *tk, MJSPlayer *ply, MJSGui *gui, int tmp_wk_num){
	...
	// 流局フラグ
	// ----------------------------------------
	tk->kyoku[tk->kyoku_index].yk.ryukyoku_flg  = true;
	tk->kyoku[tk->kyoku_index].yk.yakuman_count = 0;
	tk->kyoku[tk->kyoku_index].yk.yakucount     = 0;
	...
	if(strcmp(gui->wk_str[tmp_wk_num+3], "fanpai") == 0 ){
		// 流局：荒牌
		tk->kyoku[tk->kyoku_index].yk.ryukyoku_stat = HOWANPAI;
	}else if(strcmp(gui->wk_str[tmp_wk_num+3], "sufonrenta") == 0 ){
		// 流局：四風連打
		tk->kyoku[tk->kyoku_index].yk.ryukyoku_stat = SUFURENDA;
	}else if(strcmp(gui->wk_str[tmp_wk_num+3], "suchareach") == 0 ){
		// 流局：四人リーチ
		tk->kyoku[tk->kyoku_index].yk.ryukyoku_stat = RIICHI4PLY;
	}else{
		// 流局：不明
		tk->kyoku[tk->kyoku_index].yk.ryukyoku_stat = UNKNOWNRKK;
	}
	...
	for(int tmp_ryukyoku_num = tmp_wk_num+4; tmp_ryukyoku_num < gui->wk_str_count; tmp_ryukyoku_num++){
		...
		if(strcmp(gui->wk_str[tmp_ryukyoku_num], "tenpais") == 0 ){
			...
			// テンパイ人数の初期化
			tk->kyoku[tk->kyoku_index].yk.ply_tempai_count = 0;

			// プレーヤごとのテンパイ確認
			for(int ply_i = 0; ply_i < PLAYER_MAX; ply_i++){
				if(strcmp(gui->wk_str[tmp_ryukyoku_num+1+ply_i], "true") == 0 ){
					tk->kyoku[tk->kyoku_index].yk.ply_tempai_flg[(tmp_ply_id_0_seki + ply_i)%4] = true;
					tk->kyoku[tk->kyoku_index].yk.ply_tempai_count++;
				}else{
					tk->kyoku[tk->kyoku_index].yk.ply_tempai_flg[(tmp_ply_id_0_seki + ply_i)%4] = false;
				}
			}
		}
		...
		if(strcmp(gui->wk_str[tmp_ryukyoku_num], "deltas") == 0 ){
			for(int ply_i = 0; ply_i < PLAYER_MAX; ply_i++){
				tk->kyoku[tk->kyoku_index].yk.kyoku_score[(tmp_ply_id_0_seki + ply_i)%4] = atoi(gui->wk_str[tmp_ryukyoku_num+1+ply_i]);
			}
		}
