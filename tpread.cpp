...
void MJSReadTplog::Set_tkinfo_kyoku_score(MJSTkinfo *tk, int kyoku_index){
	...
	// 得点初期化
	for(int ply_i = 0; ply_i < 4; ply_i++){
		tk->kyoku[kyoku_index].yk.kyoku_score[ply_i]=0;
	}
	...
	while(mode == 0){
		...
		if ( strcmp( wk_str[wk_index], "+" ) == 0){
			...
			tk->kyoku[kyoku_index].yk.kyoku_score[0]=atoi(wk_str[wk_index+1]);
			wk_index++;
		}else if(strcmp(wk_str[wk_index], tk->plyname[1]) == 0){
			tk->kyoku[kyoku_index].yk.kyoku_score[1]=atoi(wk_str[wk_index+1]);
			wk_index++;
		}else if(strcmp(wk_str[wk_index], tk->plyname[2]) == 0){
			tk->kyoku[kyoku_index].yk.kyoku_score[2]=atoi(wk_str[wk_index+1]);
			wk_index++;
		}else if(strcmp(wk_str[wk_index], tk->plyname[3]) == 0){
			tk->kyoku[kyoku_index].yk.kyoku_score[3]=atoi(wk_str[wk_index+1]);
			wk_index++;
		}
		// wk_indexを+1
		wk_index++;
...
void MJSReadTplog::Set_tkinfo_agariinfo(MJSTkinfo *tk, int kyoku_index){
	...
	// 変数定義
	int mode=0;
	tk->kyoku[kyoku_index].yk.yakucount=0;
	...
	while(mode == 0){
		...
		if ( strcmp( wk_str[wk_index], "+" ) == 0){
			...
			// 役情報の投入
			wsprintf(tk->kyoku[kyoku_index].yakuname[tk->kyoku[kyoku_index].yk.yakucount], "%s", wk_str[wk_index]);
			tk->kyoku[kyoku_index].yk.yakucount++;
...
void MJSReadTplog::Set_tkinfo_dora(MJSTkinfo *tk, int kyoku_index){
	...
	// 卓情報設定 - ドラ設定(仮置き)
	// ----------------------------------------
	tk->kyoku[kyoku_index].dora[0] = 34;
