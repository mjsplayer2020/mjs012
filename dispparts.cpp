...
void MJSDisplayParts::DispOpeningTitle(int ver1, int ver2, int ver3, int ver4, int ver5){
	...
	// タイトル用のバージョン表示
	// DrawFormatString(800, 300, GetColor(255,255,255), "Ver. %d.%d.%dpre.%d.%d", ver1, ver2, ver3, ver4, ver5);
...
// パーツ表示・プレート表示
/* ---------------------------------------------------------------------------------------------- */
void MJSDisplayParts::DispPltline(bool plt_mode[]){

	// プレート表示
	for(int tmp_i = 0; tmp_i < PLTLINE_COUNT ; tmp_i++ ) {

	    if( plt_mode[tmp_i] == true ) {
			DrawGraph       ( SPACE_XSIZE+(PLT_ICON_XSIZE+SPACE_XSIZE)*tmp_i,    PLT_Y_STAT, pltimg, TRUE );
			DrawFormatString( SPACE_XSIZE+16+(PLT_ICON_XSIZE+SPACE_XSIZE)*tmp_i, PLT_Y_STAT+7, GetColor(255,255,255), plt_name[tmp_i]);
		}
...
// パーツ表示・鳴きありなしプレート
/* ---------------------------------------------------------------------------------------------- */
void MJSDisplayParts::DispPltNakiAriNashi(bool plt_naki_stat){

	// プレート表示
	DrawGraph( PLT_NAKIARI_X_STAT, PLT_Y_STAT, pltimg, TRUE );

	// 文字表示
	if( plt_naki_stat == true ) {
		DrawFormatString( PLT_NAKIARI_X_STAT+16, PLT_Y_STAT+7, GetColor(255,255,255), plt_name[PLT_NAKIARI]);
	}else{
		DrawFormatString( PLT_NAKIARI_X_STAT+16, PLT_Y_STAT+7, GetColor(255,255,255), plt_name[PLT_NAKINASHI]);
	}
...
void MJSDisplayParts::DispGameModeButton(){
	...
	// ボタン表示
	DrawGraph( BUT01_X_STAT, BUT_Y_STAT, but_green, TRUE );
	// DrawGraph( BUT02_X_STAT, BUT_Y_STAT, but02, TRUE );
	// DrawGraph( BUT03_X_STAT, BUT_Y_STAT, but03, TRUE );
	DrawGraph( BUT04_X_STAT, BUT_Y_STAT, but_green, TRUE );
	DrawGraph( THINFO_BUT_X_STAT, BUT_Y_STAT, but_blue, TRUE );
	DrawGraph( TKINFO_BUT_X_STAT, BUT_Y_STAT, but_blue, TRUE );
	DrawGraph( PLYINFO_BUT_X_STAT, BUT_Y_STAT, but_yellow, TRUE );
	DrawGraph( QUIT_BUT_X_STAT, BUT_Y_STAT, but_red, TRUE );
...
// ドラ表示
/* ---------------------------------------------------------------------------------------------- */
void MJSDisplayParts::DispDora(int x, int y, int doracount, int dora[], bool dora_aka[]){
	...
	for(int i = 0; i < 5 ; i++ ) {
		...
		// ドラ表示
	    if( i == 0 ) {
			// DispHai( DORA_XSIZE+24*i, 5, dora[0], dora_aka[0], false, 0, 0);
			DispHai( x+24*i, y, dora[0], dora_aka[0], false, 0, 0);
		}else{
			DispHai( x+24*i, y, 0, false, false, 0, 0);
		}
...
void MJSDisplayParts::DispHai(int x, int y, int hai, bool aka, bool hai_turn, int haimode, int haicolor){
	...
	if ( hai == 0 ){
		...
		// ブレンドモード - 黄色(濃いめ)
		}else if( haimode == DISP_HAI_COLOR_NUM_DEEP_YELLOW ){
